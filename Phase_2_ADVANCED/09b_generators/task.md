...
2.  Use the `yield` keyword to return one line at a time to the caller.
3.  Pause the loop after each yield.
4.  Demonstrate usage: Use a `for` loop to consume your generator. Print `"Processing: X"` for each item.
